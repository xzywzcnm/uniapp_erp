    ...
    <view class="normalPadding">
      <view>
      	<view class="uni-form-item uni-column">
      		...
      		<input class="uni-input"  focus v-model.trim="searchKeyName" placeholder="请输入采购单号、SKU、跟踪号、供应商名称" />
      	</view>
      </view>
...
export default {
  data () {
    return {
      ...
      date: '起始日期',
      lastDate: '结束日期',
      array: ['全部', '未全部到货', '全部到货', '等待到货', '部分到货等待剩余', '部分到货不等待剩余', '已作废'],
      index: 0,
      cleanIndex: 0,
  ...
  methods: {
    search () {
      ...
      if (v.cleanIndex === '3') {
        cleanIndex = 4
      } else if (v.cleanIndex === '4') {
        cleanIndex = 5
      } else if (v.cleanIndex === 0) {
        cleanIndex = ''
      } else {
        cleanIndex = v.cleanIndex
      }
      uni.redirectTo({
      	url: '/pages/workspace/purchaseList' + '?type=' + '1' + '&searchKey=' + v.searchKeyName + '&orderStatus=' + index + '&purchaseDateFrom=' + date + '&purchaseDateTo=' + lastDate + '&clearingform=' + cleanIndex
      })
    },
