<template>
  <view :class="wrap">
    ...
    <view class="containerWrap">
      <view class="listWrap">
        <view class="listItem" v-for="(item, index) in list" :key="index" @click="gotoDetails(item.specialStockinId, item.specialStockinCode)">
          <view class="subList">
            <view class="d-flex justify-content-start">
              入库单号：<text>{{item.specialStockinCode}}</text>
            </view>
            <view class="d-flex justify-content-start">
              货品种类：<text>{{item.skuCount}}</text>
            </view>
            <view class="d-flex justify-content-start">
              创建人：<text>{{item.createName}}</text>
            </view>
            <view class="d-flex justify-content-start">
              创建日期：<text>{{item.createdDate}}</text>
            </view>
            <view class="d-flex justify-content-start">
              入库原因：<text>{{item.stockinReasonName }}</text>
            </view>
...
export default {
  ...
  methods: {
    ...
    gotoDetails(specialStockinId,specialStockinCode) {
      /**
       * @Description:
       * @author gan
       * @date: 2020/11/23 17:40
       * @params: specialStockinId 其它入库id
       * @params: specialStockinCode 入库单号
      */
      let v = this
      uni.navigateTo({
        url: '/pages/workspace/otherWarehousingDetails?isMoreGoodsShelf='+v.isMoreGoodsShelf + '&specialStockinId='+ specialStockinId + '&warehouseId=' + v.warehouseId + '&specialStockinCode=' + specialStockinCode
      })
    },
    getList() {
      let v = this
